      ...
      <div class="card">
        ...
        <form @submit.prevent="saveSettings" class="form">
          ...
          <div class="projections">
            ...
            <div
              v-for="p in projections" :key="p.label"
        ...
        <form @submit.prevent="saveTheme" class="form">
          ...
          <div class="field">
            ...
            <div class="color-row">
              ...
              <input type="text" v-model="tf.primary_color" class="input mono" style="width:96px;flex-shrink:0" maxlength="7" />
              <div class="presets">
                <button
                  v-for="c in PRESETS" :key="c" type="button"
                  class="preset"
                  :class="{ 'preset-active': tf.primary_color === c }"
                  :style="{ background: c }"
                  @click="tf.primary_color = c"
                  :title="c"
                ></button>
              </div>
            </div>
          ...
          <div class="preview-block">
            ...
            <div class="preview-card">
              ...
              <button
                type="button" class="preview-cta"
                :style="{ background: tf.primary_color }"
              >Entrar</button>
            </div>
          </div>
        ...
        <form @submit.prevent="saveDomain" class="form">
          ...
          <div class="dns-guide">
            ...
            <ol class="dns-steps">
              ...
              <li>
                Nome: <code>crm</code>
                → Valor: <code>cname.vercel-dns.com</code>
              </li>
          ...
          <div class="domain-status">
            <span
              class="status-dot"
              :class="localOrg?.custom_domain ? 'status-on' : 'status-off'"
            ></span>
            <span>
...
// ─── Estado local da org ─────────────────────────────────────────────────────
// Cópia local do org que atualiza imediatamente com a resposta de cada PATCH,
// sem depender do fetchProfile() que pode falhar por RLS (403 no Supabase).
...
// ─── Sync central ─────────────────────────────────────────────────────────────
// Chamada tanto pelo watch(org) na carga inicial
// quanto diretamente pela resposta da API após cada save.
...
// Sync na carga inicial (quando org é preenchido pelo composable)
watch(org, (o) => {
  if (!o) return
  syncFromOrg(o)
...
// ─── Projeções (números de ouro) ──────────────────────────────────────────────
const projections = computed(() => {
  const tm = sf.ticket_medio || 1
  const mm = sf.meta_mensal  || 0
  const f  = Math.ceil(mm / tm)
  const rr = Math.ceil(f  / 0.4)
  const rm = Math.ceil(rr / 0.4)
  const ce = Math.ceil(rm / 0.027)
  const ld = Math.ceil(ce / 0.45)
  return [
    { label: 'Fechamentos / mês',         value: f },
    { label: 'Reuniões realizadas / mês', value: rr },
    { label: 'Reuniões marcadas / mês',   value: rm },
    { label: 'Contatos efetivos / mês',   value: ce },
    { label: 'Ligações discadas / mês',   value: ld },
    { label: 'CE por dia útil',           value: Math.ceil(ce / 22) },
    { label: 'Ligações por dia útil',     value: Math.ceil(ld / 22) },
  ]
})

// ─── Handlers de save ─────────────────────────────────────────────────────────
// Cada handler usa a resposta do PATCH para atualizar a UI imediatamente
// (syncFromOrg), evitando depender do fetchProfile que pode falhar por RLS.
...
async function saveSettings() {
  ...
  try {
    const updated = await $fetch<any>('/api/settings', {
      ...
      body: {
        name:         sf.name,
        meta_mensal:  sf.meta_mensal,
        ticket_medio: sf.ticket_medio,
      },
    })
    syncFromOrg(updated)
    fetchProfile().catch(() => {}) // best-effort; não bloqueia UI
...
async function saveTheme() {
  ...
  try {
    ...
    // Aplica o tema imediatamente no cliente
    applyTheme({
      primary_color: tf.primary_color,
      product_name:  tf.product_name,
      logo_url:      tf.logo_url    || null,
...
.input:hover, .select:hover { border-color: #d4d4d4; }
