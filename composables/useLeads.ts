...
// Elimina requisições duplicadas e padroniza os optimistic updates.
...
export const useLeads = () => {
  ...
  async function toggleFU(leadId: string, attemptIndex: number) {
    ...
    // Optimistic: atualiza state local imediatamente
  ...
  // ── Patch campo genérico (sem optimistic — usado no detail panel) ──────
  async function patchLead(leadId: string, payload: Partial<Lead>) {
    const data = await $fetch<Lead>(`/api/leads/${leadId}`, {
      method: 'PATCH',
      body: payload,
    })
  ...
  function exportCSV() {
    ...
    const headers = [
      'Decisor', 'Empresa', 'Telefone', 'Instagram',
      'Status', 'Data Retorno', 'Reunião', 'FU Feitos',
      'Num. Vendedores', 'Ponte', 'Informações', 'Cadastrado em',
    ]
